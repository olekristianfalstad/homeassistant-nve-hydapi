...
def check_dialogs(browser, name):
    ...
    try:
        ...
        action = page.get_by_text("Behold valgte m\u00e5leserier", exact=False).filter(visible=True).first
        ...
        action.click()
        page.get_by_text("Legg til m\u00e5leserier", exact=True).filter(visible=True).click()
        ...
        expect(page.get_by_text(re.compile("D\u00f8gn")).filter(visible=True)).to_be_visible()
